...
//---------------------------------------------------------------------------
__fastcall TfrmFuncion::TfrmFuncion(TComponent* Owner)
        : TForm(Owner)
{
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmFuncion::pntIntervalosPaint(TObject *Sender)
{
  ...
  for( i = 0; i <= Count(Intervalos)-1;i++ ){
    ...
    pntIntervalos->Canvas->Rectangle(offset+(Aux.Min-a)*w/(b-a),h/2-6, offset+(Aux.Max-a)*w/(b-a),h/2-1);
    pntIntervalos->Canvas->Brush->Color = clWindow;
    pntIntervalos->Canvas->TextOut(offset+(Aux.Min-a)*w/(b-a)-Canvas->TextWidth(FormatFloat("0.##",Aux.Min))/2, h/2+1, FormatFloat("0.##",Aux.Min));
    pntIntervalos->Canvas->TextOut(offset+(Aux.Max-a)*w/(b-a)-Canvas->TextWidth(FormatFloat("0.##",Aux.Max))/2, h/2+1, FormatFloat("0.##",Aux.Max));
  }
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmFuncion::btnAplicarClick(TObject *Sender)
{
  ...
  if( !Eval.Evaluate(txtExpresion->Text) ){
    if( (Eval.LastError() == eSintaxis) || !CheckTokens(txtExpresion->Text)){
      Application->MessageBox("Error de sintaxis, verifique la expresión","Error en definición", MB_OK+MB_ICONERROR);
      txtExpresion->SetFocus();
      return;}
  }else
    if( !CheckTokens(txtExpresion->Text) ){
      Application->MessageBox("Error de sintaxis, verifique la expresión","Error en definición", MB_OK+MB_ICONERROR);
      txtExpresion->SetFocus();
      return;}
  if( IsDouble(txtMin->Text) )
    ...
    Application->MessageBox("Se aceptan solo números reales...",
      "Error en definición",MB_OK+MB_ICONERROR);
    txtMin->SetFocus();
    return;}
  if( IsDouble(txtMax->Text) )
    ...
    txtMax->SetFocus();
    return;}
  strcpy(Info.Expresion,txtExpresion->Text.c_str());
  ...
  switch( CheckNewInterval(Intervalos,Info) ){
    case erIntervaloAbsurdo:{
      Application->MessageBox("Intervalo absurdo, no se agregará...",
              "Error en definición",MB_OK+MB_ICONERROR);
      if( e )
        Add(Intervalos,Aux);
      txtMax->SetFocus();
    ...
    case erSuperposicion:{
      Application->MessageBox("Se superpone el intervalo, no se agregará...",
              "Error en definición",MB_OK+MB_ICONERROR);
      if( e )
        Add(Intervalos,Aux);
      txtMin->SetFocus();
...
{
  if( (lstIntervalos->ItemIndex != -1) &&
        (lstIntervalos->ItemIndex != Count(Intervalos)) )
    if(Application->MessageBox("Estoy por eliminar un intervalo. ¿Continúo?...",
      "Eliminación",MB_YESNO+MB_ICONQUESTION) == mrYes )
    {
      Delete(Intervalos,Find(Intervalos,lstIntervalos->ItemIndex)->Info);
      ...
      Application->MessageBox("Eliminación cancelada","Información",
        MB_OK+MB_ICONINFORMATION);
  else
    Application->MessageBox("Seleccione un intervalo...","Error en definición",
        MB_OK+MB_ICONERROR);
  pntIntervalos->Repaint();
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmFuncion::cboGrosorDrawItem(TWinControl *Control,
      int Index, TRect &Rect, TOwnerDrawState State)
{
  ilsGrosor->Draw(cboGrosor->Canvas,Rect.Left+(Rect.Right-50)/2,Rect.Top,
        Index,true);
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmFuncion::btnAceptarClick(TObject *Sender)
{
  if( Count(Intervalos) == 0 ){
    Application->MessageBox("La función no contiene intervalos",
        "Error en definición",MB_OK+MB_ICONERROR);
    ModalResult = mrNone;}
  else{
    ModalResult = mrOk;}
...
{
  const char *sTitle = "Error en definición";
  ...
  if( IsDouble(edtValue->Text) )
  {
    ...
    if( Info != NULL )
      if( Eval.Evaluate(Info->Info.Expresion) )
      {
         ...
         {
           case eNoError:
             Msg = Format("x = %s\ny = %g",ARRAYOFCONST((edtValue->Text,result)));
             Application->MessageBox(Msg.c_str(), "Evaluación", bInfo);
             break;
           case eDivCero:
             Application->MessageBox("División sobre cero", sTitle, bError);
             break;
           case eDominio:
             Application->MessageBox("Fuera del dominio de la función", sTitle, bError);
             break;
           case eSintaxis:
             Application->MessageBox("Verifique la sintaxis", sTitle, bError);
             break;
         }
      }else
      {
    ...
    else
    {
     ...
     Application->MessageBox("Ingrese un número valido", sTitle, bError);
}
//---------------------------------------------------------------------------
