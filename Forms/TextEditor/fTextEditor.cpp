...
// ---------------------------------------------------------------------------
__fastcall TfrmTextEditor::TfrmTextEditor(TComponent* Owner) : TForm(Owner) {
}
...
// ---------------------------------------------------------------------------
void __fastcall TfrmTextEditor::ParOrdenado(AnsiString Formato, long double x,
	long double y, AnsiString Additional) {
	SendMessage(rtxtTexto->Handle, EM_SCROLL, SB_LINEDOWN, 0);
	rtxtTexto->SelAttributes->Color = clBlack;
	rtxtTexto->SelAttributes->Name = "Courier New";
	rtxtTexto->SelAttributes->Size = 10;
...
void __fastcall TfrmTextEditor::rtxtTextoKeyDown(TObject *Sender, WORD &Key,
	TShiftState Shift) {
	int Lpos, Cpos, LineLength;
	Lpos = SendMessage(rtxtTexto->Handle, EM_LINEFROMCHAR,
		rtxtTexto->SelStart, 0);
	...
	sbrStatus->Panels->Items[0]->Text = "Lin " + IntToStr(Lpos + 1);
	// {Linea / Line}
	sbrStatus->Panels->Items[1]->Text = "Col " + IntToStr(Cpos + 1);
	// {Columna / Column}
	sbrStatus->Panels->Items[2]->Text = "Len " + IntToStr(LineLength + 1);
	// {Longitud de la linea / Long of line}
}
// ---------------------------------------------------------------------------
	...
	if (dlgSaveText->Execute())
		if (dlgSaveText->FilterIndex == 1) {
			if (Application->MessageBox
				(L"Guardar en formato de solo texto implica perder las caracteristicas del formato de texto enriquecido.\n¿Desea continuar?",
...
void __fastcall TfrmTextEditor::rtxtTextoKeyPress(TObject *Sender, char &Key) {
	WORD Key2;
	TShiftState Shift;
...
void __fastcall TfrmTextEditor::actImprimirExecute(TObject *Sender) {
	try {
		...
		Application->MessageBox
			(L"Para poder imprimir necesita instalar una "
			L"impresora.\nPara hacer esto, haga click en el botón Inicio, elija " "Configuración y luego Impresoras y\ndespués seleccione "
			L"Agregar impresora.", L"Error de impresión", MB_OK + MB_ICONWARNING);
	}
}
// ---------------------------------------------------------------------------
