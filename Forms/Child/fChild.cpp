        ...
        : TForm(Owner)
{
  if( Funciones.LoadFromFile(Archivo) ){
    ...
    if( frmMain->Config.ErrorArchivos )
      Application->MessageBox("Formato desconocido de archivo.\n"
      "No se continuará con la apertura del mismo.","Error en archivo",
      MB_OK+MB_ICONERROR);
    Tag = -1;//Valor necesario para evadir OnActivate, por bug de ActionList
    Close();
  }
}
...
void __fastcall TfrmChild::FormClose(TObject *Sender, TCloseAction &Action)
{
  ...
  if( Funciones.Modificado )
    switch(Application->MessageBox(Msg.c_str(),"Confirmar antes de cerrar",
...
void __fastcall TfrmChild::actNuevaFuncionExecute(TObject *Sender)
{
  ...
  if( frmFuncion->ShowModal() == mrOk ){
    ...
    strcpy(F.Nombre,frmFuncion->txtNombre->Text.c_str());
    F.Graficar = true;
...
void __fastcall TfrmChild::actEliminarFuncionExecute(TObject *Sender)
{
  ...
  if( frmSelectFunc->ShowModal() == mrOk )
    ...
    {
      if(Application->MessageBox(Format("¿Confirma la eliminación de la función \"%s\"?", ARRAYOFCONST((frmSelectFunc->cboFunciones->Text))).c_str(),
...
void __fastcall TfrmChild::actEliminarTodasExecute(TObject *Sender)
{
  if( frmMain->Config.DeleteFunction )
  {
    if(Application->MessageBox("¿Confirma la eliminación de todas las funciones?",
        "Confirmar antes de continuar",MB_YESNO+MB_ICONQUESTION) == IDYES)
    {
...
void __fastcall TfrmChild::FormActivate(TObject *Sender)
{
  ...
  if( Tag != -1 ){
    ...
    EstadoControles();
    frmMain->sbrEstado->Panels->Items[0]->Text = Funciones.Modificado?"Modificado":"No modificado";
    }
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmChild::actArbolFuncionesExecute(TObject *Sender)
{
  if( Funciones.Count > 0 ){
    ...
    Application->MessageBox("No existen funciones definidas en el proyecto.",
...
void __fastcall TfrmChild::actSelectorGraficasExecute(TObject *Sender)
{
  ...
  for( int i = 0; i < Funciones.Count; i++)
    frmSelectorGraficas->lstFunciones->Checked[i] = Funciones.Items[i].Graficar;
...
void __fastcall TfrmChild::actGuardarComoExecute(TObject *Sender)
{
  ...
  if( frmMain->SaveDialog->Execute() )
    if( Funciones.SaveToFile(frmMain->SaveDialog->FileName) ){
      if( dsgVisor->SaveToFile(ChangeFileExt(frmMain->SaveDialog->FileName,".dsg")) ){
        PathName = frmMain->SaveDialog->FileName;
        Caption = ExtractFileName(PathName);
        EstadoControles();
        frmMain->Config.DirectorioPmm = ExtractFilePath(frmMain->SaveDialog->FileName);
        frmMain->sbrEstado->Panels->Items[0]->Text = "No modificado";
      }
    }else
      if( frmMain->Config.ErrorArchivos )
        Application->MessageBox("Imposible guardar el archivo.\n"
        "No se continuará con la escritura del mismo.","Error en archivo",
        MB_OK+MB_ICONERROR);
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmChild::actGuardarExecute(TObject *Sender)
{
  if(Caption == DefaultFileName)
    ...
    if( !Funciones.SaveToFile(PathName) && dsgVisor->SaveToFile(ChangeFileExt(PathName,".dsg")) && frmMain->Config.ErrorArchivos){
      Application->MessageBox("Imposible guardar el archivo.\n"
        "No se continuará con la escritura del mismo.","Error en archivo",
        MB_OK+MB_ICONERROR);}
    else{
  ...
  if( frmMain->OpenDialog->Execute() )
    if( Funciones.ImportFunction(frmMain->OpenDialog->FileName) ){
      ...
      if( frmMain->Config.DeleteImport )
        if(Application->MessageBox("¿Desea eliminar el archivo importado?",
          "Confirmar antes de continuar",MB_YESNO+MB_ICONQUESTION) == IDYES)
          if( !DeleteFile(frmMain->OpenDialog->FileName) )
            Application->MessageBox("Imposible eliminar el archivo.",
                  "Error en archivo", MB_OK+MB_ICONERROR);
    }
    else
      Application->MessageBox("Imposible abrir el archivo.\n"
        "No se continuará con la lectura del mismo.","Error en archivo",
        MB_OK+MB_ICONERROR);
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmChild::actZoomDefaultExecute(TObject *Sender)
{
  ...
  Format("Zx: %d%% / Zy: %d%%", ARRAYOFCONST((dsgVisor->ZoomAxeX, dsgVisor->ZoomAxeY)));
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmChild::actImprimirExecute(TObject *Sender)
{
  ...
  if( frmMain->Config.CheckWhiteColor && (dsgVisor->BackGround->Color != clWhite ))
  {
    Intro = Application->MessageBox(
      "El color de fondo no es blanco, si continúa con el proceso de impresión \n"
      "seguramente consumirá mayor cantidad de tinta \n"
      "¿Desea cancelar la impresión y modificar el color de fondo?",
      "Confirmar antes de continuar",MB_YESNOCANCEL+MB_ICONQUESTION );
    switch( Intro )
    {
      case IDYES:
      ...
      case IDNO:
        try{
          ...
          Application->MessageBox(msg, "Error de impresión", MB_OK + MB_ICONWARNING);
        }
    }
  }
  else
...
void __fastcall TfrmChild::actDerivadaExecute(TObject *Sender)
{
  ...
  if( frmIngresoDatos->ShowModal() == mrOk )
  {
    ...
    if( frmFuncion->ShowModal() == mrOk ){
      if( Trim(frmFuncion->txtNombre->Text) == "" )
        frmFuncion->txtNombre->Text = AutoName();
      strcpy(frmIngresoDatos->F.Nombre,frmFuncion->txtNombre->Text.c_str());
...
{
  switch(Key){
    case VK_UP:       dsgVisor->MoveToTop();     break;
    case VK_DOWN:     dsgVisor->MoveToBottom();  break;
    case VK_LEFT:     dsgVisor->MoveToLeft();    break;
    case VK_RIGHT:    dsgVisor->MoveToRight();   break;
    case VK_ADD:      actZoomMasExecute(this);   break;
    case VK_SUBTRACT: actZoomMenosExecute(this); break;
  }
}
//---------------------------------------------------------------------------

void __fastcall TfrmChild::dsgVisorMouseMove(TObject *Sender,
      TShiftState Shift, int X, int Y)
{ 
  frmMain->sbrEstado->Panels->Items[2]->Text = Format("(%.5g;%.5g)", ARRAYOFCONST((dsgVisor->ScreenXToRealX(X), dsgVisor->ScreenYToRealY(Y))));
  frmMain->sbrEstado->Panels->Items[3]->Text = Format("Zx: %d%% / Zy: %d%%", ARRAYOFCONST((dsgVisor->ZoomAxeX, dsgVisor->ZoomAxeY)));
  if( dsgVisor->Cursor == TCursor(5) ){
    dsgVisor->SetCenter(dsgVisor->CenterX-dsgVisor->ScreenXToRealX(X)+dsgVisor->ScreenXToRealX(px),
      dsgVisor->CenterY-dsgVisor->ScreenYToRealY(Y)+dsgVisor->ScreenYToRealY(py));
    px = X;
    py = Y;
  }
}
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
  if( Button == mbLeft )
    if( dsgVisor->Cursor == 7 )
      ...
      if( dsgVisor->Cursor == 6 )
        actZoomMasExecute(Sender);
      else
        if( dsgVisor->Cursor == TCursor(4) )
        {
...
void __fastcall TfrmChild::actAreaLateralSupRevolucionExecute(
      TObject *Sender)
{
  TfrmIngresoDatos *frmIngresoDatos;
  frmIngresoDatos = new TfrmIngresoDatos(this);
...
void __fastcall TfrmChild::dsgVisorMouseUp(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
  if( dsgVisor->Cursor == TCursor(5) )
    dsgVisor->Cursor = TCursor(4);
...
void __fastcall TfrmChild::actPtsCriticos1eraDerivadaExecute(
      TObject *Sender)
{
  TfrmIngresoDatos *frmIngresoDatos;
  frmIngresoDatos = new TfrmIngresoDatos(this);
...
void __fastcall TfrmChild::actPtsCriticos2daDerivadaExecute(
      TObject *Sender)
{
  TfrmIngresoDatos *frmIngresoDatos;
  frmIngresoDatos = new TfrmIngresoDatos(this);
