...
typedef void (*FRegExt)(char* ext, char* tfile, char* desc, char* exe, int idicon);
...
void __fastcall TfrmMain::FormActivate(TObject *Sender)
{
    ...
    for(int i = 1; i <= ParamCount(); i++)
    {
        ...
        {
			Application->MessageBox(L"El archivo no se encuentra", L"Error", MB_OK + MB_ICONERROR);
		}
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TfrmMain::actConfImpresExecute(TObject *Sender)
{
  ...
  try{
	...
	Application->MessageBox(AnsiString(msg).c_str(), L"Error de impresión", MB_OK + MB_ICONWARNING);
  }
}
...
{
	ShellExecute(NULL, L"open", L"mailto:contacto@devstudios.com.ar", NULL, NULL, SW_SHOWNORMAL);
}
...
void __fastcall TfrmMain::actRegistrarseExecute(TObject *Sender)
{
    ...
    */
	...
	Application->MessageBox(msg, L"Registro", MB_OK + MB_ICONINFORMATION);
}
...
void __fastcall TfrmMain::actFullScreenExecute(TObject *Sender)
{
    if(!FullScreen)
    {
        ...
        int AWidth  = GetDeviceCaps(Canvas->Handle, HORZRES) + (Width-ClientWidth);
        int AHeight = GetDeviceCaps(Canvas->Handle, VERTRES) + (Height-ClientHeight);
...
{
	const AnsiString FileDoc = ExtractFilePath(Application->ExeName) + "doc\\Manual del usuario de MAXMATH.pdf";
	ShellExecute(Handle, L"open", FileDoc.c_str(),"",NULL,0);
}
//---------------------------------------------------------------------------

void __fastcall TfrmMain::GetMinMaxInfo(TWMGetMinMaxInfo &Msg)
{
	Msg.MinMaxInfo->ptMaxTrackSize.x = GetDeviceCaps(Canvas->Handle, HORZRES) + (Width-ClientWidth);
	Msg.MinMaxInfo->ptMaxTrackSize.y = GetDeviceCaps(Canvas->Handle, VERTRES) + (Height-ClientHeight);
}
//---------------------------------------------------------------------------
...
{
    HINSTANCE dllhandle = LoadLibrary("RegExt.dll");
    if(dllhandle)
    {
    	FIsRegExt IsRegExt = (FIsRegExt)GetProcAddress(dllhandle, "_IsRegExt");
    	if(IsRegExt && !IsRegExt(".pmm", "MaxMath File", Application->ExeName.c_str()))
        {
    		FRegExt RegExt = (FRegExt)GetProcAddress(dllhandle, "_RegExt");
    		if(RegExt)
            {
            	RegExt(".pmm", "MaxMath File", "Proyecto MaxMath", Application->ExeName.c_str(), 0);
            }
    	}
    ...
    {
    	char msg[] = "No se puedo asociar la extensión.\nLa librería 'RegExt.dll' no está disponible.";
